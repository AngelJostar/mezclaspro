<x-admin-layout>
    @php
        ...
        $selectedDeliveryLaboratoryId = (int) old('delivery_laboratory_id', 0);
        $selectedDeliveryLaboratory = $deliveryLaboratories->firstWhere('id', $selectedDeliveryLaboratoryId);
        $selectedWarehouseId = (int) old('warehouse_id', 0);
        ...
        $selectedInventoryDestination = old('inventory_destination', '');
        if (! array_key_exists($selectedInventoryDestination, $inventoryDestinations)) {
            $selectedInventoryDestination = '';
        }
                ...
                <div class="po-sheet" aria-label="Formato editable de orden de compra">
                    ...
                    <div class="po-request-destination-row" style="{{ $position(86.7, 148.6, 497.7, 11.9) }}">
                        ...
                        <select id="delivery_laboratory_id" name="delivery_laboratory_id" required
                            ...
                            <option value="">Central: Selecciona</option>
                        ...
                        <select id="warehouse_id" name="warehouse_id" required @disabled(!$selectedDeliveryLaboratory)
                            aria-label="Almac&eacute;n receptor" title="Almac&eacute;n receptor">
                            <option value="">Almac&eacute;n: Selecciona</option>
                            @foreach ($selectedDeliveryLaboratory?->warehouses ?? collect() as $warehouse)
                        ...
                        <select id="inventory_destination" name="inventory_destination" required @disabled(!$selectedWarehouse)
                            aria-label="Subalmac&eacute;n receptor" title="Inventario receptor">
                            <option value="">Subalmac&eacute;n: Selecciona</option>
                    ...
                    @foreach ([0, 1] as $slot)
                        ...
                        <div class="po-cell po-cell-description" data-item-slot="{{ $slot }}"
                            ...
                            <input data-item-field="description" type="text" disabled autocomplete="off"
                                role="combobox" aria-autocomplete="list" aria-expanded="false" aria-controls="po-product-options"
                                placeholder="Selecciona central, almacén y subalmacén" aria-label="Descripci&oacute;n de la partida">
                            <button type="button" class="po-product-toggle" data-product-toggle="{{ $slot }}"
                                disabled aria-label="Mostrar productos" title="Mostrar productos" aria-expanded="false"
                                aria-controls="po-product-menu"></button>
                        </div>
                    ...
                    <div class="po-item-pagination" style="{{ $position(95, 468.2, 220, 18) }}" aria-label="Partidas de la orden">
                        <button id="po-previous-items" type="button" title="Partidas anteriores" aria-label="Partidas anteriores"></button>
                        <span id="po-items-page" aria-live="polite"></span>
                        <button id="po-next-items" type="button" title="Partidas siguientes" aria-label="Partidas siguientes"></button>
                    </div>
                    ...
                    <div style="position:absolute; z-index:3; {{ $position(220, 715, 172, 13) }}">
                        <input id="prepared_by" name="prepared_by" type="text" readonly
                            value="{{ trim((auth()->user()?->name ?? '') . ' ' . (auth()->user()?->lastname ?? '')) }}"
    ...
    @php
        $purchaseOrderConfig = [
            ...
            'items' => $initialItems,
            'productsUrl' => route('admin.oncologicos.laboratory.purchase-orders.products', $laboratory),
        ];
    @endphp
    <script id="purchase-order-config" type="application/json">@json($purchaseOrderConfig, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT)</script>
</x-admin-layout>
