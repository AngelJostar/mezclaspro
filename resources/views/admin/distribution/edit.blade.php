<x-admin-layout>
    <div class="rounded-lg bg-white p-5 shadow-sm md:p-6">
        <header class="flex flex-col gap-4 border-b border-gray-200 pb-5 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <nav class="mb-2 text-xs font-medium text-gray-500" aria-label="Ruta de navegación">
                    ...
                    <a href="{{ route('admin.distribution.show', $distributionRoute) }}" class="hover:text-blue-700">{{ $distributionRoute->name }}</a>
                    <span class="mx-2 text-gray-300">/</span>
                    <span class="text-blue-700">Editar</span>
                </nav>
                <h1 class="text-2xl font-bold text-gray-950">Editar ruta de distribución</h1>
                <p class="mt-1 text-sm text-gray-500">Actualiza el recorrido y sus responsables.</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.distribution.update', $distributionRoute) }}" class="mt-6">
            @csrf
            @method('PUT')
