<x-admin-layout>
    ...
    <div class="mt-4 overflow-x-auto rounded border bg-white"><table class="min-w-full text-sm"><thead class="bg-slate-50 text-left"><tr><th class="p-3">Insumo / lote</th><th class="p-3">Solicitada</th><th class="p-3">Aprobada</th><th class="p-3">Surtida</th><th class="p-3">Recibida</th><th class="p-3">Disponible</th></tr></thead><tbody>
        @foreach($supplyRequest->lines as $line)
            ...
            <tr class="border-t"><td class="p-3 font-medium">@if($line->consumableLot){{ $line->consumableLot->item?->name }} · {{ $line->consumableLot->presentation }}<br><span class="text-xs text-slate-500">Lote {{ $line->consumableLot->lot ?: '—' }} · Caducidad {{ $line->consumableLot->expires_at?->format('d/m/Y') ?: '—' }}</span>@else{{ $line->supply?->diluent?->denominacion_generica }} · {{ $line->supply?->presentacion }}<br><span class="text-xs text-slate-500">Lote {{ $line->supply?->lote ?: '—' }} · Caducidad {{ $line->supply?->caducidad?->format('d/m/Y') ?: '—' }}</span>@endif</td><td class="p-3">{{ $line->requested_quantity }}</td><td class="p-3">@if($isRequested && $canManageWarehouse)<input name="lines[{{ $line->id }}][approved_quantity]" type="number" min="0" step="0.01" value="{{ $line->requested_quantity }}" class="w-24 rounded border-slate-300">@else{{ $line->approved_quantity ?? '—' }}@endif</td><td class="p-3">@if($isSupplyReady && $canManageWarehouse)<input name="lines[{{ $line->id }}][supplied_quantity]" type="number" min="0" step="0.01" value="{{ $line->approved_quantity }}" class="w-24 rounded border-slate-300">@else{{ $line->supplied_quantity ?? '—' }}@endif</td><td class="p-3">@if($isSupplied && $canReceive)<div class="flex items-center gap-2"><input name="lines[{{ $line->id }}][received_quantity]" type="number" min="0" step="0.01" value="{{ $line->supplied_quantity }}" class="w-24 rounded border-slate-300">@if($loop->first)<button type="submit" class="rounded bg-green-700 px-3 py-2 text-xs font-semibold text-white">Confirmar recepción</button>@endif</div>@else{{ $line->received_quantity ?? '—' }}@endif</td><td class="p-3">{{ $stockLot?->stock_actual ?? '—' }}</td></tr>
        @endforeach
    </tbody></table></div>
